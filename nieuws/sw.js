/* -------------------------------------------------------------
   ...
   - other same-origin GETs: stale-while-revalidate (serve cache if
     present, refresh in the background so the next load has the
     new asset)
   - /api/* is never cached — news must be fresh.
   Bump CACHE_VERSION when any cached file changes.
   ------------------------------------------------------------- */

const CACHE_VERSION = "v1";
const CACHE_NAME = `citax-nieuws-${CACHE_VERSION}`;
...
self.addEventListener("fetch", (event) => {
  ...
  // Same-origin only — let cross-origin pass through.
  const url = new URL(req.url);
  if (url.origin !== self.location.origin) return;

  // Never cache the RSS proxy — news must be fresh on every load.
